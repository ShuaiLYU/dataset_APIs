...
class WSLDataset_split(WSLDataset):
    ...
    def make_dataset(self):
        from datasets.WSLDatasets import get_cur_path
...
if  __name__=="__main__":
   ...
   # root="G:\数据集\KolektorSDD"
    ...
    #
    # transform_PIL = my_transforms.ComposeJoint([
    #         my_transforms.ToPIL(),
    #         my_transforms.GroupRandomHorizontalFlip(),
    #         my_transforms.GroupRandomVerticalFlip(),
    #         my_transforms.GroupResize(size=(1250, 500)),
    #     ])
    # # data=WSLDataset(root,return_numpy=True)
    # # data_trian = WSLDataset_train(root, return_numpy=True)
    # # data_test = WSLDataset_test(root, return_numpy=True)
    # data=WSLDataset_split(root,"class1_train5valid5","training")
    # for image, label_pixel, label, file_basename_image in data:
    #     print(image.shape)
    #     print(label_pixel.shape)
    #     #plt_utils.plt_show_imgs([image.squeeze(-1),label_pixel.squeeze(-1)])


    # save_dir="../config/cong_3_folder3_1"
    # data=KolektorSDD_split(root,save_dir,"training",transform_PIL)
    # # for image, label_pixel, label, file_basename_image in data:
    # #     print(image.shape)
    # #     print(label_pixel.shape)
    # #     plt_utils.plt_show_imgs([image.squeeze(-1),label_pixel.squeeze(-1)])
    ...
    #     "batch_size":4,
    # }
    # data_manager=utils.data.data_managers.DataManager(data,param)
    # for idx,(item1,item2,item3,item4) in enumerate(data_manager):
    #     print(idx)
    #     print(item1.shape)
    #     print(item2.shape)
    #     print(item3)
    #     print(item4)
    #
    # # data_manager=utils.data.data_managers.DataManager_valid(data,batch_size=3)
    # # for idx,(item1,item2,item3,item4) in enumerate(data_manager):
    # #     print(idx)
    # #     print(item1.shape)
    # #     print(item2.shape)
